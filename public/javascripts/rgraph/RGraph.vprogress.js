    ...
    */
    RGraph.VProgress.prototype.DrawTickMarks = function ()
    {
        ...
        if (this.Get('chart.tickmarks')) {
            ...
            var startX = this.Get('chart.labels.position') == 'left' ? (this.gutterLeft - 4) : (RGraph.GetWidth(this) - this.gutterRight);
            var endX   = this.Get('chart.labels.position') == 'left' ? this.gutterLeft : RGraph.GetWidth(this) - this.gutterRight + 4;

            this.tickInterval = this.height / this.Get('chart.numticks');
            
            var endY = this.Get('chart.tickmarks.zerostart') ? (RGraph.GetHeight(this) - this.gutterBottom) : (RGraph.GetHeight(this) - this.gutterBottom - this.tickInterval);

            this.context.beginPath();
                for (var i=this.gutterTop; i<=endY; i+=this.tickInterval) {
